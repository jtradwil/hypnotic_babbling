...
class explore_random(object):
    ...
    pos_r = [math.pi/2, 0, 3*math.pi/2, math.pi/2]
    ...
    def _run(self):
        ...
        rospy.loginfo('Go To X: %3.3f, Y: %3.3f, R: %3.3f', self.pos_x[pos_index], self.pos_y[pos_index], self.pos_r[rot_index])
        ...
        while(not(rospy.is_shutdown()) and (run == 1)):
            ...
            if(status == 1):
                ...
                rospy.loginfo('Retry X: %3.3f, Y: %3.3f, R: %3.3f', self.pos_x[pos_index], self.pos_y[pos_index], self.pos_r[rot_index])
                rand_point_mover._move_to_goal(self.pos_x[pos_index], self.pos_y[pos_index], self.pos_r[rot_index])
                status = 0
            else:
                status = rand_point_mover._check_goal_status(20)
